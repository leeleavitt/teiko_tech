...
def cell_counts_calculator(
    file_name: str = "cell-count.csv", file_name_out: str = "cell-counts-relative.csv"
):
    ...
    cell_rf_df = cell_df.loc[:, cell_name_columns].apply(
        lambda x: x / cell_df["total_count"] * 100
    )
    ...
    cell_rf_df_long = cell_rf_df.melt(
    ...
    # melt the dataframe to get the desired format
    cell_df_long = cell_df[columns_to_grab].melt(
        id_vars=["sample", "total_count"], var_name="population", value_name="count"
    )

    # merge the two dataframes
    cell_df_rf_long = cell_df_long.merge(
        cell_rf_df_long, how="left", on=["sample", "total_count", "population"]
    )
...
class CellCountsBoxPlot:
    ...
    def __init__(
        self,
        file_name="cell-counts-relative.csv",
        file_name_out="cell-counts-relative_boxplot.png",
    ):
    ...
    def cell_type_t_tester(self, cell_type_to_calc: str = "b_cell"):
        ...
        b_cell_group_n = self.grouped_data.get_group(("n", cell_type_to_calc))[
            "cell_count"
        ]
        b_cell_group_y = self.grouped_data.get_group(("y", cell_type_to_calc))[
            "cell_count"
        ]
    ...
    def cell_type_treatment_box_plot(self):
        ...
        cell_ss_df_melt = cell_ss_df.melt(
            id_vars=["response"], var_name="cell_type", value_name="cell_count"
        )
        ...
        ax = sns.boxplot(
            x="cell_type", y="cell_count", hue="response", data=cell_ss_df_melt
        )
