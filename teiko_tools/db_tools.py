...
def db_loader(file_name: str = "cell-count.csv"):
    ...
    cell_df = cell_df.merge(subjects, how='left', left_on='subject', right_on='subject_name')

    # Insert data into 'samples'
    samples = cell_df[['subject_id', 'treatment_id', 'response', 'sample', 'sample_type', 'time_from_treatment_start']]
    samples.columns = ['subject_id', 'treatment_id', 'response', 'sample_name', 'sample_type', 'time_from_treatment_start']
    samples.to_sql('samples', engine, schema='research_data', if_exists='append', index=False)

    # Load samples back to get the IDs
    samples = pd.read_sql_table('samples', engine, schema='research_data')

    # Map samples to their IDs
    cell_df = cell_df.merge(samples, how='left', left_on="sample", right_on="sample_name")

    cell_counts_df = cell_df[['sample_id', 'b_cell', 'cd8_t_cell', 'cd4_t_cell', 'nk_cell', 'monocyte']].drop_duplicates()
    cell_counts_df.to_sql('cell_counts', engine, schema='research_data', if_exists='append', index=False)
